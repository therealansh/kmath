...
private class ComplexBuffer<out T>(factory: BufferFactory<T>, override val size: Int, init: (Int) -> Complex<T>) :
    Buffer<Complex<T>> {
 ...
 */
public fun <T> Buffer.Companion.complex(
    factory: BufferFactory<T>,
    size: Int,
    init: (Int) -> Complex<T>,
): Buffer<Complex<T>> = ComplexBuffer(factory, size, init)

private class MutableComplexBuffer<T> private constructor(
 ...
 * specified [init] function.
 */
public fun <T> MutableBuffer.Companion.complex(
    factory: MutableBufferFactory<T>,
    size: Int,
    init: (Int) -> Complex<T>,
